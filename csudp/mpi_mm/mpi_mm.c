...
int main (int argc, char *argv[]) {
   ...
   if (taskid == MASTER) {
      ...
      for (dest=1; dest<=numworkers; dest++) {
         ...
         offset = offset + rows;
	      printf("A matrix partition sent to worker:  %d\n", dest);
	      fflush(stdout);
      }
      ...
      // Send b-matrix data to the worker tasks
      mtype = FROM_MASTER;
      for (dest=1; dest<=numworkers; dest++) {
         printf ("Sending b rows to %d\n", dest);
         MPI_Send(&b, NCA*NCB, MPI_DOUBLE, dest, mtype, MPI_COMM_WORLD);
	      printf("B matrix sent to worker:  %d\n", dest);
	      fflush(stdout);
      }
   ...
   if (taskid > MASTER) {
      ...
      printf ("Matrix A partition received from master !\n");
      ...
      printf ("Rx matrix-b from master \n");
      fflush(stdout);
