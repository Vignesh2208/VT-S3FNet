...
void run_autotest(char *testset)
{
	...
	move_s comp_move;
	struct timeval start_tstamp, stop_tstamp;
...
*/
	...
	gettimeofday(&stop_tstamp, NULL);
    double time_taken = ((double)stop_tstamp.tv_sec + (double)stop_tstamp.tv_usec / 1e6 - ((double)start_tstamp.tv_sec  + (double)start_tstamp.tv_usec / 1e6)); 
