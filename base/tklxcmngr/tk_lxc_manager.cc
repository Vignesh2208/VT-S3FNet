...
int LxcManager::readAndProcessNxtPacket(LXC_Proxy * proxy, char * buffer) {
    ...
    pair<int, unsigned int> res = analyzePacket(buffer, nread, &ethT, &isTCP);
    ...
    if (packet_status == PARSE_PACKET_SUCCESS_IP) {
        ...
        if (proxy->eqTracerID == 1) {
            int slot = arrivalTime / (10 * MS_IN_US);

            while (pktSendCounts.size() <= slot)
                pktSendCounts.push_back(0);	
            pktSendCounts[slot] ++;

            if (isTCP) {
                while (bytesSentCounts.size() <= slot)
                    bytesSentCounts.push_back(0);	
                bytesSentCounts[slot] += payload_len;
            }
        }
...
char * LxcManager::GetStatsJson(void) {
    ...
    cJSON * pkt_send_counts = NULL;
    cJSON * bytes_send_counts = NULL;
    ...
    pkt_send_counts = cJSON_CreateArray();
    if (pkt_send_counts == NULL)
    goto end;

    bytes_send_counts = cJSON_CreateArray();
    if (bytes_send_counts == NULL)
    goto end;

        cJSON_AddItemToObject(stats, "overheads", oheads);
    cJSON_AddItemToObject(stats, "burst_lengths", exec_burst_lengths);
    cJSON_AddItemToObject(stats, "pkt_send_counts", pkt_send_counts);
    cJSON_AddItemToObject(stats, "bytes_send_counts", bytes_send_counts);
    ...
    for (int i = 0; i < pktSendCounts.size(); i++) {
        cJSON * new_count_obj;
        new_count_obj = cJSON_CreateObject();
        if (new_count_obj == NULL) {
            goto end;
        }
        cJSON * idx = cJSON_CreateNumber(i);
        cJSON * value = cJSON_CreateNumber(pktSendCounts[i]);

        if (!idx || !value)
            goto end;
        
        cJSON_AddItemToObject(new_count_obj, "idx", idx);
        cJSON_AddItemToObject(new_count_obj, "value", value);
        cJSON_AddItemToArray(pkt_send_counts, new_count_obj);
    }

    for (int i = 0; i < bytesSentCounts.size(); i++) {
        cJSON * new_count_obj;
        new_count_obj = cJSON_CreateObject();
        if (new_count_obj == NULL) {
            goto end;
        ...
        cJSON * idx = cJSON_CreateNumber(i);
        cJSON * value = cJSON_CreateNumber(bytesSentCounts[i]);
        ...
        cJSON_AddItemToObject(new_count_obj, "idx", idx);
        cJSON_AddItemToObject(new_count_obj, "value", value);
        cJSON_AddItemToArray(bytes_send_counts, new_count_obj);
    }
