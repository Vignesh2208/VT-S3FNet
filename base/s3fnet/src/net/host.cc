...
void Host::initLxcProxy(s3f::dml::Configuration* cfg)
{
	char* lxcstr = (char*) cfg->findSingle("isEmulated");

	if (!lxcstr)
	{
		isEmulated = false; // host isnt to be emulated
	}
	else
	{
		if (s3f::dml::dmlConfig::isConf(lxcstr))
			error_quit("ERROR: invalid lxcID attribute.\n");
		isEmulated = true;
	}

	this->__timeline->simCtrl = inNet()->getTopNet()->getSimInterface()->get_timeline_interface()->lm;
	//hello_peer = owner_net->getNameService()->nhi2ip(hello_peer_nhi);

	if (isEmulated)
	{
		LxcManager* simCTRL =
				inNet()->getTopNet()->getSimInterface()->get_timeline_interface()->lm;

		string Proxy_NHI = nhi.toStlString();

		LXC_Proxy* lxcProxyPtr = new LXC_Proxy(Proxy_NHI, getDefaultIP(), simCTRL, this->__timeline);
		lxcProxyPtr->ptrToHost = (void*)this;

		//lxcProxyPtr->printInfo();

		proxy = lxcProxyPtr;

		simCTRL->insertProxy(lxcProxyPtr);
	}
	else
	{
		proxy = NULL;
