...
parser.add_argument('--fraction_servers', type=float, required=False,
                    ...
                    default=0.1)
parser.add_argument('--emu_flow_mean_arrival_us', type=int, default=1000000)
...
def get_lxc_cfg_string(params):
    ...
    for lan_no in range(0, int(TOTAL_LANS/2)):
        ...
        for _, server_netid in enumerate(lan_emu_netids[dest_lan]):
            server_netids.append(server_netid)


    for lan_no in range(0, int(TOTAL_LANS/2)):
        dest_lan = (lan_no + int(TOTAL_LANS / 2))

        for _, cli_netid in enumerate(lan_emu_netids[lan_no]):
            server_idx = random.randint(0,len(server_netids) - 1)
            #print ("Server idx = ", server_idx, " num servers = ", len(server_netids))
            server_netid = server_netids[server_idx]
            
            cliCmd = f"{tgenCmdPath} client {cli_netid} {server_netid}:0 7891 {muArrivalUs} {txSizeKb} > {logDir}/client{cli_netid}.log"
            
            cli_settings = f"""
        settings [ lxcNHI {cli_netid}:0 ttnProject "{ttnProject}" cmd "{cliCmd}" ]"""
        ...
        for _, server_netid in enumerate(lan_emu_netids[dest_lan]):
            ...
            server_settings = f"""
        settings [ lxcNHI {server_netid}:0 ttnProject "{ttnProject}" cmd "{serverCmd}" ]"""
...
def main():
    ...
    print (f"Generating for TCP-RAND LA-Enabled: {args.enable_lookahead}, Num Emu Hosts Per Lan: {args.num_emu_hosts_per_lan} ")
    ...
    if total_num_timelines > 1:
        log_dir = f"TCP_RAND_{lookahead_status}_nemus_{total_num_timelines - 1}_poisson_mu_{args.emu_flow_mean_arrival_us}_txsize_{args.transfer_size_kb}"
    else:
        log_dir = "TCP_FullSim"


    tgenCmd = f"{home}/VT-S3FNet/csudp/tcp_rand/rand_tgen"
