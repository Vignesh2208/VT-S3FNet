...
parser.add_argument('--k', type=int, required=False, help='k-ary fat-tree topology',
                    default=16)
parser.add_argument('--exp_type', type=str, default='mm')
parser.add_argument('--incast_period_us', type=int, default=1000000) 
parser.add_argument('--incast_bsize_kb', type=int, default=8)         
...
def get_lxc_cfg_string(params, k, num_emu_hosts):
    ...
    nEmus = params['nemus']
    incastTimePeriodUs = params['incastPeriodUs']
    incastBsizeKb = params['incastBurstSizeKb']
    incastPortStart = 7891
    settings_string = ""

    if ttnProject != 'incast_tgen':
        rank = 0
        for pod_no in range(0, k):
        ...
        for pod_no in range(0, k):
            for emu_netid in pod_emu_netids[pod_no]:
                if rank == 0:
                    server_netid = emu_netid
                    fullCmd = f"{mpiCmdPath} server {rank} {emu_netid}:0 {incastPortStart} {nEmus - 1} > {logDir}/server.log"
                else:
                    fullCmd = f"{mpiCmdPath} client {rank} {server_netid}:0 {incastPortStart + rank - 1} {incastTimePeriodUs} {incastBsizeKb} > {logDir}/client{rank}.log"
                lxc_settings = f"""
            settings [ lxcNHI {emu_netid}:0 ttnProject "{ttnProject}" cmd "{fullCmd}" ]"""
                settings_string += lxc_settings
                rank += 1
...
def generate_pod_sim_network(pod_no, k, fraction_servers):
    ...
    print ("Pod = ", pod_no, " Num sim hosts = ", num_sim_hosts_in_pod, " Num Emu Hosts = ", num_emu_hosts_in_pod)
    ...
    if num_sim_tcp_clients_added == 0 and num_sim_tcp_servers_added > 0:
        podDict = f"""
    podNet{pod_no} [
        {router_cfg}
        host 																	# Host 1:0
...
def main():
    ...
    params = {
        ...
        'nemus': total_num_timelines - 1,
        'incastPeriodUs': args.incast_period_us,
        'incastBurstSizeKb': args.incast_bsize_kb
    }
